...
const well = new Vue({
  ...
  methods: {
    stickImages() {
      ...
      loadImage.click();
      loadImage.addEventListener("change", this.getImages, false);
    },
    getImages(e) {
      // Lấy ảnh gửi
  ...
  methods: {
    ...
    handleScroll(e) {
      $(document).ready(() => {
        ...
        scrollPosition >= scrollLimit - 180 ? (this.isEnd = true) : "";
        // if (scrollPosition >= scrollLimit - 180)  {
        //   this.isEnd = true;
        // }
      });
    },
    openComment(post_Id) {
      let getCommentArea = document.getElementById("comment-area" + post_Id);
      // Bật mở ổ comment bài viêt
      let state = getCommentArea.style.display;
